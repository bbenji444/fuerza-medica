...
- **sucursales**: SELECT abierto para `authenticated` y `anon`, solo `activa = true`.
- **categorias**: SELECT abierto para `authenticated` y `anon` (sin filtro); INSERT/UPDATE/DELETE solo `es_admin()` (agregado para "Gestionar categorías" en Inventario — antes solo tenía SELECT, toda alta era por SQL directo, ver `add-rls-categorias-alta-baja.sql`).
...
"Stock bajo" = `existencia <= inventario_maximo / 3` (no usa `inventario_minimo`). Contador + reporte PDF/Excel. **"+ Agregar producto"** crea producto + fila de inventario en las 3 sucursales (existencia inicial solo en la sucursal activa). **"Borrar"** verifica que no tenga cotizaciones/ventas/promociones asociadas antes de borrar `inventario`+`productos`. Edición/alta/baja solo admin_general. **"+ Imagen"**: sube a Storage (bucket `productos`, `{producto_id}.{ext}`, `upsert:true`), guarda URL con `?t=timestamp` en `productos.imagen_url`. "Quitar" limpia el campo (no borra el archivo).

**Columna "Estado" + botón Activar/Desactivar** (solo admin_general): en vez de borrar un producto para quitarlo del catálogo público, se puede desactivar (`productos.activo = false`) — no toca cotizaciones/ventas/promociones existentes, solo dejar de venderlo/mostrarlo hacia adelante, y se puede reactivar en cualquier momento. Badge "Activo"/"Inactivo" visible para todos los roles; filtro "Activos e inactivos / Solo activos / Solo inactivos" en la barra de filtros. No requirió ninguna política RLS nueva — `productos` ya tenía UPDATE gateado por `es_admin()` (se reutiliza el mismo permiso que usa "Editar").

**Botón "Categorías"** (solo admin_general): modal para crear/editar/borrar categorías y subcategorías sin salir de Inventario. Formulario simple (nombre + categoría padre opcional); la lista de abajo agrupa cada categoría principal con sus subcategorías indentadas debajo, cada una con "Editar"/"Borrar" inline. Reglas: solo 2 niveles (una subcategoría no puede tener categoría padre que a su vez sea subcategoría, y una categoría con subcategorías propias no puede convertirse en subcategoría de otra), nombre duplicado en el mismo nivel se bloquea, y "Borrar" verifica que no tenga subcategorías ni productos/promociones asignados antes de eliminar. Requiere la política RLS de `add-rls-categorias-alta-baja.sql` (ver sección RLS arriba).
